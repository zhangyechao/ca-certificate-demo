namespace WebApi
{
    ...
    {
        public RSAKeyMapping(string appId, string serverPub, string serverPri)
        {
            this.AppId = appId;
            this.ServerPublicKey = serverPub;
            this.ServerPrivateKey = serverPri;
        }
        ...
        public static string? GetServerPublicKeyByAppId(string appId)
        {
            var rsaKey = GetAll().FirstOrDefault(x => x.AppId == appId);
            return rsaKey?.ServerPublicKey;
        }

        public static void SetClientPublicKeyByAppId(string appId, string clientPublicKey)
        {
            var rsaKey = GetAll().FirstOrDefault(x => x.AppId == appId);
            if (rsaKey != null)
            { 
                rsaKey.ClientPublicKey = clientPublicKey;
            }
        }
